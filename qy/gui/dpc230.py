...
class dpc230_settings(wx.Dialog):
    ...
    def build(self, parent):
        ...
        wx.Dialog.__init__(
            self, parent, title='DPC230 Options', size=(350, 300))
        ...
        for i in range(len(defaults)):
            d = misc.timeBinSlider(
                self, qy.util.alphabet_upper[i], defaults[i])
...
class gui_head(coincidence_counting.gui_head):
    ...
    def populate_left_panel(self):
        ...
        self.status = wx.StaticText(
            self.left_panel, label='DPC230', style=wx.ST_NO_AUTORESIZE)
