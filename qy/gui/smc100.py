...
class motor_block(wx.Panel):
    ...
    def __init__(self, parent, controller_address, state):
        ...
        self.nudge_left_button.Bind(
            wx.EVT_BUTTON, lambda x: self.nudge(sign=-1))
        self.nudge_right_button.Bind(
            wx.EVT_BUTTON, lambda x: self.nudge(sign=1))
    ...
    def nudge(self, sign):
        ...
        move_request = {
            'controller_address': self.controller_address, 'position': new_position}
...
class gui_head(wx.Frame):
    ...
    def populate(self, motor_model):
        ...
        for controller_address, actuator in motor_model.items():
            self.blocks[controller_address] = motor_block(
                self, controller_address, actuator)
            self.mainsizer.Add(
                self.blocks[controller_address], 0, wx.ALL, border=5)
...
class gui:
    ...
    def __init__(self):
        ...
        self.gui = Process(
            target=gui_head, name='gui_head', args=(their_pipe,))
        self.gui.start()
