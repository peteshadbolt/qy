...
parser.add_argument('dpc_file',  type=str,
                    help='The filename of an SPC file to process')
parser.add_argument('pattern',  type=str,
                    nargs='+', help='A start/stop pair, e.g. "AB"')
parser.add_argument('-t',  '--integration_time',
                    default=0, type=float, help='Integration time, in seconds')
parser.add_argument('-w',  '--histogram_width',
                    default=20, type=float, help='Histogram width, in ns')
parser.add_argument('-g',  '--make_graph', nargs='?', const=True,
                    default=False, type=bool, help='Should I make a graph?')
parser.add_argument('-o',  '--output_filename',
                    default=None, type=str, help='Where to write data/graphs')
...
def compute_curves(dpc_file, patterns):
    ...
    for start, stop in patterns:
        ...
        start = lookup[start.lower()]
        stop = lookup[stop.lower()]
