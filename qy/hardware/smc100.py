...
human_actuator_names={'LTA-HL': 'High load linear actuator', 'LTA-HS': 'High speed linear actuator',  'PR50CC': 'Rotation stage'}
...
class smc100_actuator:
    ...
    def identify_stage(self):
        ...
        self.send('ID?')
        self.stage_identifier=self.serial.readline()[4:].strip().split('_')[0]
        self.human_identifier=human_actuator_names[self.stage_identifier]
        self.tag='%s%s' % (self.controller_address, tag_table[self.stage_identifier])
...
class smc100:
    ...
    def open_serial_port(self, serial_port=None):
        ...
        self.serial.port=int(qy.settings.get('motors.com')) if serial_port==None else serial_port
        ...
        self.callback(('status', 'Connecting to SMC100 on COM%d...' % (self.serial_port+1)))
        self.serial_open=False
        self.serial.open()
        self.serial_open=True
        self.callback(('status', 'Connected to COM%d.' % (self.serial_port+1)))

    
    def validate_serial_port(self):
        ''' Validates the connection to the serial port '''
        self.validated=False
        if not self.serial_open: return
    ...
    def build_model(self):
        ...
        for controller_address in range(MAX_CONTROLLERS):
            ...
            if len(state)>0:
                actuator = smc100_actuator(controller_address, self.serial, self.callback)
                self.callback(('status', 'Found an actuator (%s)' % actuator.human_identifier))
