...
class fpga:
    ...
    def setMode(self, combination=0, mode=0):
        ...
        if combination >= 0 and combination <= 13:
            self.modes[13 - combination] = mode
    ...
    def writeDelays(self):
        ...
        if self.serial.isOpen():
            ...
            # Output the modes for the combinations in blocks of 4 pairs of
            # bits
...
def test_callback(s):
    print 'got this information from the fpga: %s' % s

if __name__ == '__main__':
    # run some tests
    f = fpga(callback=test_callback)
    print f.count()
