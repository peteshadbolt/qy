...
class detection_model:
    ...
    def build(self, scheme_string):
        ...
        while len(self.available_detectors) > 0 \
            ...
            if ndetectors == 1:
                self.add_detector(mode)
            if ndetectors > 1:
                self.add_splitter(ndetectors, mode)
            mode += 1
        self.detector_map = {d.label: d for d in self.detectors}

    def add_splitter(self, ndetectors, mode):
        ''' add a mode '''
        if len(self.available_splitters[ndetectors]) == 0:
            return
...
if __name__ == '__main__':
    ...
    # iterate over all the probabilitiy amplitudes that we would need to
    # simulate
    ...
    # iterate over all three-photon events, where we only care about modes,
    # with efficiencies
