...
is_divide=re.compile(r'\A[A-P]{1,16}[A-P]{1,16}\Z')
...
def parse_once(string):
    ...
    if is_fpga.match(string): return (get_fpga_pattern(string), FPGA)
    if is_number_4x4.match(string): return (get_number_pattern(string), NUMBER)
    if is_number_8x2.match(string): return (get_number_pattern(string), NUMBER)
    if is_special.match(string): return (string, SPECIAL)
    if is_divide.match(string):
        a=string.split('/')[0]
        b=string.split('/')[1]
        return (get_fpga_pattern(a),get_fpga_pattern(b),DIVIDE)
...
def parse_pattern_list(pattern_list, filt=True):
    ...
    if not filt: return patterns                                        # stop filtering if asked
    patterns=tuple(set(patterns))                                    # remove duplicates
    patterns=filter(lambda x: x!=None, patterns)
    return patterns