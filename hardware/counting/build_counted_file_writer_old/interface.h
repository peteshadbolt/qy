...
void write_scan_npoints(int n) {write_triplet(SCAN_NPOINTS, n, 0);}
...
// Saves the data to disk and returns the count rates that we are intereseted in
int stop_integrating(int paused) {
	...
	if (write_to_disk && (paused==0)) {write_nonzero_patterns_to_disk();}
